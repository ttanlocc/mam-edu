...
function GieoSidebar({ activeId }) {
  ...
  // Mobile bottom nav items (4 tabs, always same regardless of sidebar tab state)
  const MOB_NAV = [
    { id: 'today',   href: '#/today',   gly: '◉', lbl: 'Hôm nay',
      match: id => id === 'today' },
    { id: 'week',    href: '#/week',    gly: '▤', lbl: 'Tuần này',
      match: id => id === 'week' },
    { id: 'roadmap', href: '#/roadmap', gly: '⚐', lbl: 'Lộ trình',
      match: id => id === 'roadmap' },
    { id: 'writing', href: '#/writing', gly: '✎', lbl: 'Bài tập',
      match: id => ['writing','speaking','reading','listening','anki','feedback','mock'].includes(id) },
  ];
  ...
  return (
    <>
      ...
      <nav className="mob-nav">
        {MOB_NAV.map(m => (
          <a key={m.id} href={m.href} className={`mn-item${m.match(activeId) ? ' active' : ''}`}>
            <span className="mn-gly">{m.gly}</span>
            <span className="mn-lbl">{m.lbl}</span>
          </a>
        ))}
      </nav>
    </>
